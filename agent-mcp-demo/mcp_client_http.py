#!/usr/bin/env python3
"""
MCP Client 示例 (HTTP/SSE 模式)
演示如何通过 HTTP/SSE 连接 MCP Server 并调用 Tools、Resources 和 Prompts
"""
...
async def run_http_client():
    ...
    print("=" * 60)
    print("🚀 MCP HTTP Client 演示 - 电商工单数据分析")
    print("=" * 60)
    print("\n📡 正在通过 HTTP/SSE 连接 MCP Server...")
    print("   服务端点: http://localhost:8000/mcp")

    # 使用 HTTP/SSE 方式连接 Server
    # streamable_http_client 返回 3 个值: read, write, session_id_callback
    async with streamable_http_client(url="http://localhost:8000/mcp") as (
        ...
        async with ClientSession(read, write) as session:
            ...
            # ========================================
            print("\n" + "=" * 60)
            print("🎯 示例 2: 调用 Tool - 查询 'open' 状态的工单")
            print("=" * 60)
            ...
            # ========================================
            print("\n" + "=" * 60)
            print("🎯 示例 3: 调用 Tool - 查询 '物流查询' 类型工单")
            print("=" * 60)

            result = await session.call_tool(
                "query_tickets_by_type", {"issue_type": "物流查询"}
            )
            print("\n📋 查询结果:")
            for content in result.content:
                if content.type == "text":
                    tickets = json.loads(content.text)
                    print(f"找到 {len(tickets)} 个物流查询工单:\n")
                    for ticket in tickets:
                        print(f"  工单号: {ticket['ticket_no']}")
                        print(f"  客户: {ticket['customer_name']}")
            ...
            # ========================================
            print("\n" + "=" * 60)
            print("🎯 示例 4: 调用 Tool - 获取单个工单详情 (TK2024005)")
            print("=" * 60)

            result = await session.call_tool(
                "get_ticket_detail", {"ticket_no": "TK2024005"}
            )
            print("\n📄 工单详情:")
            for content in result.content:
                if content.type == "text":
