#!/usr/bin/env python3
"""
电商工单数据分析 MCP Server (HTTP/SSE 模式)
使用 FastMCP 框架构建，通过 HTTP 提供服务
"""
...
def get_ticket_statistics() -> str:
    ...
    cursor.execute("SELECT COUNT(*) FROM tickets WHERE DATE(created_at) = ?", (today,))
...
if __name__ == "__main__":
    ...
    print("🚀 启动电商工单数据分析 MCP Server (HTTP 模式)...")
    print("\n服务端点:")
    print("  - Streamable HTTP: http://localhost:8000/mcp")
    print("\n支持的 Tools/Resources/Prompts 与 stdio 版本相同")
    print("\n" + "=" * 50)

    # 使用 FastMCP 内置的 HTTP 传输方式运行
    # transport 可以是: "stdio", "http", "streamable-http", "sse"
